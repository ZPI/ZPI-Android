...
public class Database extends SQLiteOpenHelper {
	...
	private static final String CREATE_TABLE_ACTIONS_SIMPLE = "CREATE TABLE " + ACTIONS_SIMPLE + "(" + AS_ID
		+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + AS_SPEED_TYPE + " INTEGER, " + AS_VALUE_TYPE + " INTEGER, "
		+ AS_VALUE + " DOUBLE, " + AS_ORDER_NUMBER + " INTEGER" + ")";
	private static final String CREATE_TABLE_ACTIONS_ADVANCED = "CREATE TABLE " + ACTIONS_ADVANCED + "(" + AA_ID
		+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + AA_TYPE + " INTEGER, " + AA_DISTANCE_VALUE + " DOUBLE, "
		+ AA_TIME_VALUE + " DOUBLE, " + AA_PACE_VALUE + " DOUBLE, " + AA_ORDER_NUMBER + " INTEGER" + ")";
	...
	public boolean insertSingleRun(SingleRun singleRun) {
		...
		if (isInsertOk) {
			...
			long subRunNumber = 1;
			LinkedList<LinkedList<Pair<Location, Long>>> traceWithTime = singleRun.getTraceWithTime();
			for (LinkedList<Pair<Location, Long>> subRun : traceWithTime) {
				for (Pair<Location, Long> singlePointWithTime : subRun) {
					isInsertOk = isInsertOk && insertRunPart(db, singlePointWithTime, runID, subRunNumber);
				}
				subRunNumber++;
			}
			if (isInsertOk) {
	...
	private boolean insertRunPart(SQLiteDatabase db, Pair<Location, Long> singlePointWithTime, long runID,
		long subRunNumber) {
		ContentValues cv = new ContentValues();
	...
	private LinkedList<LinkedList<Pair<Location, Long>>> getTraceForRunID(long runID, SQLiteDatabase db) {
		String[] columns = { PWT_ID, PWT_RUN_NUMBER, PWT_SUB_RUN_NUMBER, PWT_LONGITUDE, PWT_LATITUDE, PWT_ALTITUDE,
			PWT_TIME_FROM_START };
		LinkedList<LinkedList<Pair<Location, Long>>> trace = null;
		
		Cursor cursor = db.query(POINTS_WITH_TIME, columns, PWT_RUN_NUMBER + " = ?", new String[] { runID + "" }, null,
			null, PWT_SUB_RUN_NUMBER);
		if (cursor.moveToFirst()) {
	...
	public long insertWorkout(Workout workout) {
		...
		if (isInsertOK = workoutID != -1) {
			...
			for (WorkoutAction workoutAction : actions) {
				...
				isInsertOK = isInsertOK && insertWorkoutAction(db, workoutAction, workoutID, index) != -1;
				index++;
			}
		}
		if (isInsertOK) {
	...
	private long insertWorkoutAction(SQLiteDatabase db, WorkoutAction workoutAction, long workoutID, int index) {
		...
		if (workoutAction instanceof WorkoutActionSimple) {
			...
			actionID = insertSimpleWorkoutAction(db, (WorkoutActionSimple) workoutAction, index);
		}
		else if (workoutAction instanceof WorkoutActionAdvanced) {
			workoutType = WorkoutAction.ACTION_ADVANCED;
			actionID = insertAdvancedWorkoutAction(db, (WorkoutActionAdvanced) workoutAction, index);
		}
		boolean isInsertOK = actionID != -1;
		if (isInsertOK) {
			isInsertOK = insertWorkoutIDActionID(db, workoutID, actionID, workoutType);
		}
		if (!isInsertOK) {
			actionID = -1;
		}
	...
	private long insertSimpleWorkoutAction(SQLiteDatabase db, WorkoutActionSimple workoutAction, int index) {
		ContentValues cv = simpleWorkoutActionToContentValues(workoutAction, index);
		
		return db.insert(ACTIONS_SIMPLE, null, cv);
	}
	
	private ContentValues simpleWorkoutActionToContentValues(WorkoutActionSimple workoutAction, int index) {
		ContentValues cv = new ContentValues();
	...
	private long insertAdvancedWorkoutAction(SQLiteDatabase db, WorkoutActionAdvanced workoutAction, int index) {
		ContentValues cv = advancedWorkoutActionToContentValues(workoutAction, index);
		
		return db.insert(ACTIONS_ADVANCED, null, cv);
	}
	
	private ContentValues advancedWorkoutActionToContentValues(WorkoutActionAdvanced workoutAction, int index) {
		ContentValues cv = new ContentValues();
	...
	public Workout getWholeSingleWorkout(long workoutID) {
		...
		if (workout != null && (simpleActions != null || advancedActions != null)) {
	...
	private List<WorkoutAction> mergeSimpleAdvancedActionsOrdered(List<WorkoutActionSimple> simpleActions,
	...
	private List<WorkoutActionSimple> getSimpleWorkoutActions(SQLiteDatabase db, long workoutID) {
		String query = "SELECT " + AS_ID + ", " + AS_SPEED_TYPE + ", " + AS_VALUE_TYPE + ", " + AS_VALUE + ", "
			+ AS_ORDER_NUMBER + " FROM " + ACTIONS_SIMPLE + " WHERE " + AS_ID + " IN " + "(" + "SELECT " + WA_ACTION_ID
			+ " FROM " + WORKOUTS_ACTIONS + " WHERE " + WA_WORKOUT_ID + "=?" + " AND " + WA_ACTION_TYPE + "=?" + ")";
		Cursor cursor = db.rawQuery(query, new String[] { workoutID + "", WorkoutAction.ACTION_SIMPLE + "" });
	...
	private WorkoutActionSimple getSimpleWorkoutActionFromCursor(Cursor cursor) {
		WorkoutActionSimple simple = new WorkoutActionSimple(cursor.getInt(1), cursor.getInt(2), cursor.getDouble(3));
		simple.setID(cursor.getLong(0));
		simple.setOrderNumber(cursor.getInt(4));
		return simple;
	}
	
	private List<WorkoutActionAdvanced> getAdvancedWorkoutActions(SQLiteDatabase db, long workoutID) {
		String query = "SELECT " + AA_ID + ", " + AA_TYPE + ", " + AA_DISTANCE_VALUE + ", " + AA_TIME_VALUE + ", "
			+ AA_PACE_VALUE + ", " + AA_ORDER_NUMBER + " FROM " + ACTIONS_ADVANCED + " WHERE " + AA_ID + " IN " + "("
			+ "SELECT " + WA_ACTION_ID + " FROM " + WORKOUTS_ACTIONS + " WHERE " + WA_WORKOUT_ID + "=?" + " AND "
			+ WA_ACTION_TYPE + "=?" + ")";
		Cursor cursor = db.rawQuery(query, new String[] { workoutID + "", WorkoutAction.ACTION_ADVANCED + "" });
	...
	private WorkoutActionAdvanced getAdvancedActionFromCursor(Cursor cursor) {
		WorkoutActionAdvanced advanced = new WorkoutActionAdvanced(cursor.getInt(1), cursor.getDouble(2),
			cursor.getDouble(4), cursor.getLong(3));
	...
	public boolean deleteWorkoutAction(long workoutID, long actionID) {
		SQLiteDatabase db = getReadableDatabase();
		String[] columns = new String[] { WA_ID, WA_WORKOUT_ID, WA_ACTION_ID, WA_ACTION_TYPE };
		boolean isDeleted = false;
		Cursor cursor = db.query(WORKOUTS_ACTIONS, columns, WA_WORKOUT_ID + "=? AND " + WA_ACTION_ID + "=?",
			new String[] { workoutID + "", actionID + "" }, null, null, null);
		if (cursor.moveToFirst()) {
			int actionType = cursor.getInt(3);
			String[] queryString = new String[] { actionID + "" };
			if (actionType == WorkoutAction.ACTION_ADVANCED) {
				isDeleted = db.delete(ACTIONS_ADVANCED, AA_ID + "=?", queryString) != 0;
			}
			else if (actionType == WorkoutAction.ACTION_SIMPLE) {
				isDeleted = db.delete(ACTIONS_SIMPLE, AS_ID + "=?", queryString) != 0;
			}
			isDeleted = isDeleted
				&& db.delete(WORKOUTS_ACTIONS, WA_WORKOUT_ID + "=? AND " + WA_ACTION_ID + "=?", new String[] {
	...
	public boolean deleteWorkout(long workoutID) {
		...
		db.delete(WORKOUTS_ACTIONS, WA_WORKOUT_ID + "=?", new String[] { workoutID + "" });
	...
	public boolean updateWorkout(Workout workout) {
		...
		db.beginTransaction();
		isOK = updateBasicWorkoutInfo(db, workout);
		isOK = isOK && updateWorkoutActions(db, workout);
	...
	private boolean updateWorkoutAction(SQLiteDatabase db, long workoutId, WorkoutAction action, int index) {
		...
		switch (action.getActionType()) {
			case WorkoutAction.ACTION_SIMPLE:
				isOK = updateWorkoutActionSimple(db, workoutId, (WorkoutActionSimple) action, index);
				break;
			case WorkoutAction.ACTION_ADVANCED:
				isOK = updateWorkoutActionAdvanced(db, workoutId, (WorkoutActionAdvanced) action, index);
				break;
			default:
				isOK = false;
				break;
	...
	public boolean updateWorkoutAction(long workoutID, WorkoutAction action, int index) {
		SQLiteDatabase db = getWritableDatabase();
		boolean isOK = updateWorkoutAction(db, workoutID, action, index);
		db.close();
		return isOK;
	}
}
