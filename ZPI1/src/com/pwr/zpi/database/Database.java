...
public class Database extends SQLiteOpenHelper {
	...
	private static final String CREATE_TABLE_POINTS_WITH_TIME = "CREATE TABLE " + POINTS_WITH_TIME + "(" +
	...
	public boolean insertSingleRun(SingleRun singleRun) {
		...
		if (isInsertOk) {
			long subRunNumber = 1;
			for (LinkedList<Pair<Location, Long>> subRun : singleRun.getTraceWithTime()) {
	...
	private boolean insertRunPart(SQLiteDatabase db, Pair<Location, Long> singlePointWithTime, long runID, long subRunNumber) {
		ContentValues cv = new ContentValues();
	...
	public List<SingleRun> getAllRuns() {
		...
		if (cursor.moveToFirst()) {
            ...
            do {
            	SingleRun singleRun = getSingleRunForCursor(cursor, db);
            	runs.add(singleRun);
            }
	...
	private LinkedList<LinkedList<Pair<Location, Long>>> getTraceForRunID(
		...
		String[] columns = {PWT_ID, PWT_RUN_NUMBER, PWT_SUB_RUN_NUMBER, PWT_LONGITUDE, 
