...
public class DatabaseTest extends AndroidTestCase {
	...
	public void testGet() {
		...
		for (int i = 0; i < testSubjects; i++) {
			boolean isOK = db
					.insertSingleRun(prepareDataSingleRun((i + 3) * 3));
			assertTrue(isOK);
		}
	...
	private void checkActions(List<WorkoutAction> actions, int creationI) {
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			if (i % 2 == 0) {
				...
				checkSimpleAction(creationI, i,
						(WorkoutActionSimple) actions.get(i));
			} else {
				assertTrue(actions.get(i) instanceof WorkoutActionAdvanced);
				checkAdvancedAction(creationI, i,
						(WorkoutActionAdvanced) actions.get(i));
			}
		}
	}

	private void checkSimpleAction(int creationI, int i,
			WorkoutActionSimple simple) {
		assertEquals(
				i % 6 == 0 || i % 6 == 3 ? WorkoutAction.ACTION_SIMPLE_SPEED_FAST
						: i % 6 == 1 || i % 6 == 4 ? WorkoutAction.ACTION_SIMPLE_SPEED_STEADY
								: WorkoutAction.ACTION_SIMPLE_SPEED_SLOW,
				simple.getSpeedType());
		assertEquals(
				i % 6 == 3 || i % 6 == 4 || i % 6 == 5 ? WorkoutAction.ACTION_SIMPLE_VALUE_TYPE_DISTANCE
						: WorkoutAction.ACTION_SIMPLE_VALUE_TYPE_TIME,
				simple.getValueType());
		assertEquals((double) creationI * i, simple.getValue());
	}

	private void checkAdvancedAction(int creationI, int i,
			WorkoutActionAdvanced advanced) {
		assertEquals(
				i % 3 == 0 ? WorkoutAction.ACTION_ADVANCED_TYPE_DISTANCE_PACE
						: i % 3 == 1 ? WorkoutAction.ACTION_ADVANCED_TYPE_PACE_TIME
								: WorkoutAction.ACTION_ADVANCED_TYPE_TIME_DISTANCE,
				advanced.getType());
